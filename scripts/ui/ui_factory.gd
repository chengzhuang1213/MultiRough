...
static func build_title_plate(text: String, minimum_size: Vector2, font_size: int) -> PanelContainer:
	...
	plate.add_theme_stylebox_override("panel", CampaignSatchelUIThemeScript.make_title_style())
	...
	label.add_theme_color_override("font_outline_color", CampaignSatchelUIThemeScript.TEXT_OUTLINE)
...
static func build_separator(minimum_size: Vector2) -> HSeparator:
	...
	separator.add_theme_stylebox_override("separator", CampaignSatchelUIThemeScript.make_separator_style())
