...
func basic_attack(combat, origin: Vector2, direction: Vector2, _length: float, _half_width: float, damage: float, is_critical: bool, attacker: PlayerController) -> void:
	combat.fire_player_arrow(origin, direction, damage, attacker, 560.0, 1.2, 18.0, "", 672.0 * attacker.get_attack_range_multiplier(), Vector2.ZERO, false, is_critical)
	combat.game._spawn_line_skill_effect(origin, direction, 54.0, Color(1.0, 0.88, 0.42, 0.35), 0.08)

func use_q(combat, origin: Vector2, direction: Vector2, _length: float, _half_width: float, damage: float, is_critical: bool, attacker: PlayerController) -> void:
	...
	var projectile: PlayerProjectile = combat.fire_player_arrow(
		...
		Vector2(152.0, 48.0), true, is_critical
	)
...
func use_e(combat, origin: Vector2, direction: Vector2, _length: float, _half_width: float, damage: float, is_critical: bool, attacker: PlayerController) -> void:
...
func use_f(combat, origin: Vector2, direction: Vector2, damage: float, duration: float, is_critical: bool, attacker: PlayerController) -> void:
	combat.add_arrow_rain(origin, direction, damage, minf(duration, 5.0), attacker, is_critical)
