...
func use_q(combat, origin: Vector2, direction: Vector2, length: float, half_width: float, damage: float, is_critical: bool, attacker: PlayerController) -> void:
	...
	combat.damage_and_pull_enemies(origin, radius, damage * 0.85 * damage_multiplier, attacker, is_critical)
...
func use_e(combat, origin: Vector2, direction: Vector2, length: float, half_width: float, damage: float, is_critical: bool, attacker: PlayerController) -> void:
	if attacker.get_upgrade_level("warrior_e_shield") > 0:
		combat.throw_warrior_shield(origin, direction, damage, attacker, is_critical)
		return
	attacker.activate_warrior_counter(3.0)
	combat.add_warrior_counter_field(attacker, damage * attacker.warrior_counter_pulse_multiplier, 3.0, is_critical)
...
func use_f(combat, _origin: Vector2, _direction: Vector2, damage: float, duration: float, is_critical: bool, attacker: PlayerController) -> void:
	combat.start_blade_ultimate(attacker, damage, minf(duration, 6.0), is_critical)
