...
func use_e(combat, origin: Vector2, direction: Vector2, length: float, half_width: float, damage: float, attacker: PlayerController) -> void:
	if attacker.get_upgrade_level("lancer_e_spear") > 0:
		combat.throw_lancer_spear(origin, direction, damage, attacker, attacker.get_upgrade_level("lancer_e_return") > 0)
		return
	...
	combat.lancer_dash_spin(attacker, direction, distance, damage)
	if attacker.get_upgrade_level("lancer_e_double_sweep") > 0:
		combat.schedule_lancer_second_sweep(attacker, damage)
